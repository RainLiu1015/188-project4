...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC] = domain1
    variableDomainsDict[GHOST1] = domain1
    variableDomainsDict[GHOST0] = domain1
    variableDomainsDict[OBS0] = domain2
    variableDomainsDict[OBS1] = domain2
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for varToBeEliminated in eliminationOrder:
            # use joinFactorsByVariable() to get (factors not joined&resulting factor from joinFactors)
            factors, joinedFactor = joinFactorsByVariable(factors, varToBeEliminated)
            # print(factors)
            if len(joinedFactor.unconditionedVariables()) > 1: # if there's only one unconditioned var, discard it
                joinedFactor = eliminate(joinedFactor, varToBeEliminated)
                factors.append(joinedFactor)

        returnFactor = joinFactors(factors) # join all the factors to make the final factor
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for i in range(len(keyList)):
            if prob <= randomNum < prob + self[keyList[i]]:
                return keyList[i]
            prob += self[keyList[i]]
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if ghost is in jail, noisyDistance is guaranteed to be Node
        # which means: P(noisyDis = None|pacmanPos, ghostPos = jailPos) = 1
        # P(noisyDis = None|pacmanPos, ghostPos != jailPos) = 0;
        if noisyDistance == None:
            return float(ghostPosition == jailPosition)
        if ghostPosition == jailPosition:
            return float(noisyDistance == None)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        numMod = numParticles % numPos
        n = int(numParticles / numPos)
